...
let suite root =
  ...
  let expected_families = ["Higher"; "Point"; "Small"; "Tiny"; "Types"; "Up"] in
  let installed = Global.StringMap.bindings globals.families |> List.map fst in
  let* () = require "functor instance inventory changed"
    (installed = expected_families) in
  let* () = require "functor entry inventory changed"
    (List.length rows = 81 && Global.StringMap.cardinal globals.entries = 81) in
  let members = ["eqTrans"; "eqCongr"; "Functor"; "functorObj";
    "functorMap"; "functorMapId"; "functorMapComp"; "idFunctor"; "compFunctor"] in
  let* () = List.fold_left (fun acc instance -> let* () = acc in
    List.fold_left (fun acc member -> let* () = acc in
      require ("missing functor member: " ^ instance ^ "_" ^ member)
  ...
  let computations = ["objectIdentity", one; "objectForward", zero;
    "objectReverse", one; "mapIdentity", one; "mapForward", one;
    "mapReverse", zero; "mapNested", two] in
  ...
  let negatives = ["wrong-object"; "wrong-endpoint"; "wrong-identity-law";
    "wrong-composition-law"; "missing-law"; "erased-object"; "wrong-level-arity"] in
  let* listed = Mechanism_import.Io.attempt
    (fun () -> Sys.readdir (Filename.concat root "test/neg/functor")) in
  ...
  let* () = Result.fold
    ...
    ~ok:(fun _ -> Error "functor budget: expected refusal")
    ~error:(fun error -> require "functor budget: wrong refusal"
...
let () =
  ...
  Result.fold (suite root)
    ~ok:(fun (entries, instances, computations, negatives) ->
      Printf.printf "PRELUDE-FUNCTOR-OK entries=%d instances=%d computations=%d negatives=%d\n"
        entries instances computations negatives)
    ~error:(fun message -> Printf.printf "PRELUDE-FUNCTOR-FAIL %s\n" message; exit 1)
