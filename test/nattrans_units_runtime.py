"""Compare certified identity whiskering and horizontal units on three hosts."""
...
GATE = "PRELUDE-NATTRANS-UNITS-RUNTIME"
# Both gates use the existing 900-second CATEGORY watchdog.
TOTAL_BUDGET = 540
EMIT_BUDGET = 480
HOST_BUDGET = 30
FUNCTIONS = [
    (name, 203, 707) for name in
...
SOURCES = [
    ...
    "test/fixtures/prelude/nattrans-units-runtime.mech",
]


def main():
    if len(sys.argv) != 1:
        print("usage: python3 -I test/nattrans_units_runtime.py", file=sys.stderr)
        return 64
    ...
    hosts = set()
    compared = set()
    with tempfile.TemporaryDirectory(prefix="mechanism-nattrans-units-") as directory:
        work = Path(directory)
