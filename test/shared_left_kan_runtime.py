"""Compare shared left Kan computations on three hosts."""
...
GATE = "PRELUDE-SHARED-LEFT-KAN-RUNTIME"
...
def main():
    if len(sys.argv) != 1:
        print("usage: python3 -I test/shared_left_kan_runtime.py", file=sys.stderr)
        return 64
    ...
    exports = ["lanUnitResult", "lanMapResult", "lanDescFirst", "lanDescSecond",
               "lanVerticalFirst", "lanVerticalSecond"]
    seen = set()
    blocks = []
    ...
    for payload, suffix in [(37, ""), (41, "_41")]:
        # Distinct cocones choose distinct mediators; vertical composition keeps their order.
        answers = [payload + 1, payload + 3, payload + 5, payload + 13,
                   payload + 11, 2 * payload + 6]
    ...
    if selected.returncode or selected.stderr or selected.stdout != "RUNTIME-SLICE-OK\n":
        ...
        return 1
    with tempfile.TemporaryDirectory(prefix="mechanism-shared-left-kan-") as directory:
        work = Path(directory)
