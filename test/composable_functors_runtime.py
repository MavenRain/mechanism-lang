"""Compare composable functor computations on the kernel and both WASM hosts."""
...
GATE = "PRELUDE-COMPOSABLE-FUNCTORS-RUNTIME"


def main():
    if len(sys.argv) != 1:
        print("usage: python3 -I test/composable_functors_runtime.py", file=sys.stderr)
        return 64
    source = "".join((ROOT / path).read_text() for path in [
        "prelude/cat/category-core.mech",
        "prelude/cat/composable-functors.mech",
        "test/fixtures/prelude/composable-functors-runtime.mech",
    ])
    anchor = "def composableInput : Nat := 37"
    if source.count(anchor) != 1:
        print(f"{GATE} FAIL expected one payload anchor")
        return 1
    exports = ["objectValue", "mapValue", "compositionValue", "identityValue"]
    completed = 0
    hosts = set()
    ...
    with tempfile.TemporaryDirectory(prefix="mechanism-composable-functors-") as directory:
        work = Path(directory)
        for payload in [37, 41]:
            variant = work / str(payload)
            variant.mkdir()
            fixture = variant / "source.mech"
            fixture.write_text(source.replace(anchor, f"def composableInput : Nat := {payload}"))
