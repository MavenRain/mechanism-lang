"""Compare components certified by pointwise whiskering laws on three hosts."""
...
GATE = "PRELUDE-WHISKERING-LAWS-RUNTIME"
TOTAL_BUDGET = 290
EMIT_BUDGET = 240
HOST_BUDGET = 30
FUNCTIONS = [
    ('rightIdLeftAt', 200, 2607),
...
SOURCES = [
    ...
    'test/fixtures/prelude/whiskering-laws-runtime.mech',
]


def main():
    if len(sys.argv) != 1:
        print("usage: python3 -I test/whiskering_laws_runtime.py", file=sys.stderr)
        return 64
    ...
    hosts = set()
    compared = set()
    with tempfile.TemporaryDirectory(prefix="mechanism-whiskering-laws-") as directory:
        work = Path(directory)
