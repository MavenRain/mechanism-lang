...
(** The family name the expected type carries, when that type reduces to
    a left former at the mu shape.  Any other shape answers [None], and
    the caller keeps the family the name scan found. *)
let expected_family (c : Check.ctx) (expected : Value.t option) : string option =
  Option.bind expected (fun (ty : Value.t) ->
      Option.bind (Result.to_option (whnf_in c ty)) (fun (w : Value.t) ->
          Option.bind (Value.as_lan w)
            (fun ((vs, _dclo, _u) :
                   Value.t Shape.t * Value.closure * Level.t option) ->
              Option.map
                (fun ((name, _indices) : string * Value.t list) -> name)
                (Rules.as_vmu vs))))

(** The kernel resolves a constructor inside the family of the expected
    type (rules.ml mu_intro_in).  The elaborator does the same, so a
    constructor name that two families declare reads its record from the
    expected family:  its arity, its field telescope and its result
    indices.  The name scan of [find_ctor] stays as the fallback, and it
    is the only answer when no expected type is available. *)
let ctor_in (c : Check.ctx) (expected : Value.t option)
    (f : Positivity.family) (ct : Positivity.ctor) :
    Positivity.family * Positivity.ctor =
  Option.bind (expected_family c expected) (fun (name : string) ->
      Option.bind (Global.find_family name (globals_of c))
        (fun (fam : Positivity.family) ->
          Option.map
            (fun (found : Positivity.ctor) -> (fam, found))
            (Positivity.ctor_of ct.Positivity.c_name fam)))
  |> Option.value ~default:(f, ct)
...
let rec elab (c : Check.ctx) ~(expected : Value.t option) (s : Syntax.t) :
  ...
  | Syntax.SVar x ->
      ...
      |> Option.fold
           ...
           ~some:(fun (r : mu_ref) -> elab_mu_ref c ~expected r [])
  ...
  | Syntax.SApp (f, a) -> elab_app c ~expected f a
...
(** "f a" (SA-D1).  The head is inferred, so the argument is elaborated
    ...
    that type declares. *)
and elab_app (c : Check.ctx) ~(expected : Value.t option)
    (f : Syntax.t) (a : Syntax.t) : (Term.t, Error.t) result =
  let head, args = spine_of (Syntax.SApp (f, a)) [] in
  Option.bind (var_name head) (mu_ref_of c)
  |> Option.fold
       ~none:(fun () -> elab_app_point c f a)
       ~some:(fun (r : mu_ref) -> fun () -> elab_mu_ref c ~expected r args)
...
and elab_mu_ref (c : Check.ctx) ~(expected : Value.t option)
    (r : mu_ref) (args : Syntax.t list) :
    (Term.t, Error.t) result =
  match r with
  | RFam f -> elab_fam_ref c f args
  | RCtor (f, ct) -> elab_ctor_ref c ~expected f ct args
...
(** Fields determine a constructor's result indices.  The constructor
    record comes from the expected family when that family declares the
    name, as the kernel's introduction rule reads it.  A parameterized
    family reads parameter values from its expected type.  Every field
    elaborates at its declared type under those parameters and preceding
    fields.  The kernel checks the resulting introduction term. *)
and elab_ctor_ref (c : Check.ctx) ~(expected : Value.t option)
    (scan_f : Positivity.family) (scan_ct : Positivity.ctor)
    (args : Syntax.t list) : (Term.t, Error.t) result =
  let (f : Positivity.family), (ct : Positivity.ctor) =
    ctor_in c expected scan_f scan_ct
  in
  let* () =
  ...
  let* penv =
    ...
    | (_e : Quantity.t * string * Term.t) :: (_rest : Positivity.telescope) ->
        let* ty = expected
          |> Option.to_result ~none:(no_expect ("the constructor " ^ ct.Positivity.c_name)) in
        let* w = whnf_in c ty in
        let wrong = Error.Mismatch
          ("the constructor " ^ ct.Positivity.c_name ^ " expects family " ^ f.Positivity.f_name) in
        let* vs, dclo, _u = Value.as_lan w |> Option.to_result ~none:wrong in
        let* name, _indices = Rules.as_vmu vs |> Option.to_result ~none:wrong in
        if String.equal name f.Positivity.f_name then Rules.mu_param_env Check.ops c dclo
        else Error wrong
  in
  let* fields = Rules.zip ct.Positivity.c_args args
    |> Option.to_result ~none:(Error.Mismatch "constructor field arity changed during elaboration") in
  let* rev_ts, env = List.fold_left
    (fun acc (((_q, _x, ty), arg) : (Quantity.t * string * Term.t) * Syntax.t) ->
      let* rev_ts, env = acc in
      let* tyv = Eval.eval (globals_of c) env ty in
      let* term = elab c ~expected:(Some tyv) arg in
      let* value = eval_in c term in
      Ok (term :: rev_ts, value :: env))
    (Ok ([], penv)) fields in
  let ts = List.rev rev_ts in
  let* ixv =
    Rules.all_ok
      (List.map
         (fun (ri : Term.t) -> Eval.eval (globals_of c) env ri)
...
(** Elaborate constructor self references in a temporary symbolic family.
    ...
    cause. *)
let elab_family_parts ~budget globals ~arity (fm : Syntax.fam) =
  ...
  Ok (family, constructors, provisional)

let elab_family_template ~budget globals catalog ~arity fm =
  let* family, constructors, _provisional = elab_family_parts ~budget globals ~arity fm in
  Family_poly.declare ~budget globals catalog ~arity family constructors

(** Families check in source order, followed by nonrecursive members.
    The temporary globals provide expectations for constructor labels and
    dependent member bodies.  Only the checked group enters the catalog. *)
let elab_family_group ~budget globals catalog definitions ~arity group members =
  let names = List.map (fun fm -> fm.Syntax.fm_name) group
    @ List.map (fun m -> m.Syntax.rd_name) members in
  let* () = Rules.all_ok (List.map (fun name ->
    if Option.is_some (Poly.arity definitions name)
       || Option.is_some (Family_poly.arity catalog name)
...
let elab_composition ~budget globals catalog definitions ~arity ~name dependencies members =
  ...
  let* () = Rules.all_ok (List.map (fun label ->
    if String.equal label name || List.mem label names
        || Option.is_some (Poly.arity definitions label)
        || Option.is_some (Family_poly.arity catalog label)
        || Option.is_some (Global.find_family label globals)
        || (Option.is_some (Global.find label globals)
            && Option.is_none (find_ctor label globals)) then
      Error (Error.Mismatch ("the name " ^ label ^ " is already declared"))
    else Ok ()) labels) |> Result.map (fun _checks -> ()) in
  let members = List.map (fun member symbolic ->
    let local = member.Syntax.rd_name in
    if Option.is_some (Global.find local symbolic)
        || Option.is_some (Global.find_family local symbolic)
        || Option.is_some (find_ctor local symbolic) then
      Error (Error.Mismatch ("the name " ^ local ^ " is already declared"))
    else elab_decl (Check.make ~level_arity:arity symbolic budget)
      (Syntax.DDef (local, member.Syntax.rd_ty, member.Syntax.rd_body))) members in
  Family_poly.compose ~budget ~members globals catalog ~arity ~name dependencies
  ...
  List.fold_left
    ...
    ->
      ...
      (* SC-D6:  a constructor name occupies the flat namespace as its
         family name does (vendor/veil/lib/global.ml:1-5), so every
      ...
      let names = names @ (match d with
        ...
        | Syntax.DDef _ | Syntax.DAxiom _ | Syntax.DPoly _ | Syntax.DPolyMu _ | Syntax.DPolyGroup _
        | Syntax.DPolyCompose _
      ...
      | Syntax.DPolyMu (arity, fm) ->
          ...
          Ok (g, catalog, families, rows)
      | Syntax.DPolyGroup (arity, fm, companions, members) ->
          let* families = elab_family_group ~budget g families catalog ~arity
            (fm :: companions) members in
          Ok (g, catalog, families, rows)
      | Syntax.DPolyCompose (arity, name, dependencies, members) ->
      ...
      | Syntax.DSpecialize (name, levels, as_name, reuse) ->
          ...
          if Option.is_some (Family_poly.arity families name) then
            ...
            let* instances = Rules.all_ok (List.map (fun n ->
              ...
              family_names) in
            (* SC-D6, review round 1:  an installed label also joins the
               reservation of the caller globals, so it cannot take the
               name of an earlier instance, of an earlier generated
               member or of any other definition.  A name that the
               globals already hold as a constructor label stays
               allowed, because one template installs its labels again
               at every instance. *)
            let* () = Rules.all_ok (List.map (fun ctor ->
              if Option.is_some (Poly.arity catalog ctor.Positivity.c_name)
                 ...
                 || String.equal ctor.Positivity.c_name as_name
                  || List.mem ctor.Positivity.c_name (family_names @ members)
                 || Option.is_some (Global.find_family ctor.Positivity.c_name g)
                 || (Option.is_some (Global.find ctor.Positivity.c_name g)
                     && Option.is_none (find_ctor ctor.Positivity.c_name g)) then
                Error (Error.Mismatch ("the name " ^ ctor.Positivity.c_name ^ " is already declared"))
              else Ok ()) (List.concat_map (fun f -> f.Positivity.f_ctors) instances))
              |> Result.map (fun _checks -> ()) in
            let* member_rows = Rules.all_ok (List.map (fun n ->
              Global.find n installed |> Option.to_result
                ~none:(Error.Cannot_infer "specialization returned no member")
              |> Result.map (fun entry -> (n, entry))) members) in
