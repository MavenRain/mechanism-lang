...
let rec conv (ops : 'c Rules.ops) (ctx : 'c) ~(ty : Value.t) (a : Value.t) (b : Value.t)
    : (bool, Error.t) result =
  let* prop = is_prop ops ctx ty in
  (* SB-M2 site *)
  match () with
  | () when prop -> Ok true
  | () ->
...
(** Rule 1, proof irrelevance.  The probe reads back the type and asks
    ...
    a proposition as far as this rule is concerned.  A probe failure
    falls through to the other steps, except that budget exhaustion
    stops the comparison. *)
and is_prop (ops : 'c Rules.ops) (ctx : 'c) (ty : Value.t) : (bool, Error.t) result =
  ops.Rules.o_quote ctx ty
  |> Fun.flip Result.bind (fun (t : Term.t) -> ops.Rules.o_infer_univ ctx t)
  |> Result.fold ~ok:(fun level -> Ok (Level.always_zero level))
       ~error:(fun error ->
         match error with
         | Error.Budget_exhausted _ -> Error error
         | Error.Not_yet _ | Error.Parse (_, _, _) | Error.Carry _
         | Error.Unbound _ | Error.Mismatch _ | Error.Universe _ | Error.Quantity _
         | Error.Wrong_leg _ | Error.Missing_branch _ | Error.Overflow _
         | Error.Cannot_infer _ | Error.Index_not_zero _
         | Error.Index_above_universe _ | Error.Termination _ -> Ok false)
...
and structural (ops : 'c Rules.ops) (ctx : 'c) (a : Value.t) (b : Value.t) :
  ...
  first_of
    [
      (fun () -> univ_case (ops.Rules.o_budget ctx) wa wb);
      (fun () -> lit_case wa wb);
...
and univ_case (budget : Budget.t) (a : Value.t) (b : Value.t) : (bool, Error.t) result option =
  Option.bind (Value.as_univ a) (fun (l1 : Level.t) ->
      Option.map (fun (l2 : Level.t) -> Level.equal_budget budget l1 l2) (Value.as_univ b))
      ...
      Option.map
        (fun ((s2 : Value.t Shape.t), (d2 : Value.closure), (u2 : Level.t option)) ->
          ...
          if not same then Ok false
            ...
            let* levels = pack.Rules.ann_lvl_eq (ops.Rules.o_budget ctx) s1 u1 u2 in
            if levels then pack.Rules.conv_diagram ops ctx s1 d1 d2 else Ok false)
        (view b))
