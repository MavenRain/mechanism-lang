  ...
  List.fold_left
    (fun (acc : (Term.t, Error.t) result) (fr : Value.spine) ->
      ...
      | Value.SElim se ->
          ...
          (* Frozen eliminations close over s_env. Reopen their binders at
             the quotation depth before reifying motives and branches. *)
          let* motive = Option.fold ~none:(Ok None) ~some:(fun motive ->
            let arity = 1 + List.length motive.Term.m_idx in
            let vars = List.init arity (fun i -> Value.var (size + i)) in
            let* body = Rules.open_closure (ev globals)
              (Value.close se.Value.s_env motive.Term.m_body) vars in
            let* body = quote globals (size + arity) body in
            Ok (Some { motive with Term.m_body = body })) se.Value.s_motive in
          let* branches = Rules.all_ok (List.map (fun (address, leg) ->
            let* address = eval_addr globals se.Value.s_env address in
            let* address = quote_addr globals size address in
            let* leg = quote_leg globals size {
              Value.vl_binders = leg.Term.l_binders;
              vl_clo = Value.close se.Value.s_env leg.Term.l_body;
            } in
            Ok (address, leg)) se.Value.s_branches) in
          Ok
            (Term.Elim
               {
                 Term.e_shape = s';
