...
type 'c rule_pack = {
  ...
  conv_diagram :
    ...
    (bool, Error.t) result;
  ann_lvl_eq : Budget.t -> Value.t Shape.t -> Level.t option -> Level.t option ->
    (bool, Error.t) result;
...
let spi_ann_lvl_eq (_budget : Budget.t) (_s : Value.t Shape.t)
    (_u1 : Level.t option) (_u2 : Level.t option) : (bool, Error.t) result =
  Ok true
...
let coll_ann_lvl_eq (budget : Budget.t) (s : Value.t Shape.t)
    (u1 : Level.t option) (u2 : Level.t option) : (bool, Error.t) result =
  as_vcoll s
  |> Option.fold ~none:(Ok true) ~some:(fun (n : int) ->
         if Int.equal n 0 then
           Level.equal_budget budget
  ...
  |> Option.fold ~none:(Ok false) ~some:(fun ((n : string), (_ix : Value.t list)) ->
         ...
         |> Result.fold
              ~ok:(fun (fam : Positivity.family) ->
                Ok
                  (Level.always_zero fam.Positivity.f_level
...
let mu_large (n : string) (fam : Positivity.family) (mlvl : Level.t) :
    (unit, Error.t) result =
  match () with
  | () when Level.always_positive fam.Positivity.f_level -> Ok ()
  | () when Level.always_zero mlvl -> Ok ()
