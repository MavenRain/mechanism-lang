...
type ctx = {
  ...
  size : int;
  budget : Budget.t;
  level_arity : int;  (** Global universe binders, never term binders. *)
  levels_checked : bool;  (** Internal descendants of one validated raw term. *)
}

let make ?(level_arity = 0) (globals : Global.t) (budget : Budget.t) : ctx =
  { globals; env = []; locals = []; size = 0; budget; level_arity; levels_checked = false }

let scope (c : ctx) (t : Term.t) : (ctx, Error.t) result =
  if c.levels_checked then Ok c
  else
    let* () = Level_scope.check c.budget c.level_arity t in
    Ok { c with levels_checked = true }
...
and infer_node (c : ctx) (mode : Quantity.t) (t : Term.t) : (Value.t * Quantity.usage, Error.t) result =
  ...
  | Term.Univ l ->
      if Level.in_scope c.level_arity l then
        Ok (Value.VUniv (Level.succ l), Quantity.empty)
...
and check_former (c : ctx) (s : Term.t Shape.t) (diagram : Term.t) (expected : Value.t)
  ...
  let* same = Level.equal_budget c.budget l l0 in
  if same then Ok ()
  else
    Error
      (Error.Universe
...
let check_decl_at ~(arity : int) (globals : Global.t) (budget : Budget.t) (d : decl) :
...
let check_decl globals budget d = check_decl_at ~arity:0 globals budget d

(** Check a template universally, without exposing an ordinary global entry. *)
let check_scheme globals budget ~arity d : (unit, Error.t) result =
  if arity < 0 then Error (Error.Universe "a universe parameter arity must be nonnegative")
  else Result.map (fun _entry -> ()) (check_decl_at ~arity globals budget d)
  ...
  let* actx =
    List.fold_left
      (fun (acc : (ctx, Error.t) result) ((q, x, ty) : Quantity.t * string * Term.t) ->
        let* c' = acc in
        let* l = infer_univ c' ty in
        let* below = Level.le_budget c'.budget l fam.Positivity.f_level in
        let* () =
          if below then Ok ()
